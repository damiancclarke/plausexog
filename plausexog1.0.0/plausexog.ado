...
*! Version 0.2.1 mayo 9, 2014 @ 13:22:36
...
1.0.0: Completed beta testing.  Output complete.
*/
...
if length("`graph'")!=0 {
	if "`method'"=="uci" {
		...
		if length("`graphopts'")==0 {
			...
			*/ xtitle("{stSymbol: d}") title("Union of Confidence Interval Approach") /*
	...
	if "`method'"=="ltz" {
		...
		if length("`graphopts'")==0 {
			local graphopts ytitle(""Estimated Beta for `graph'"") /*
			*/ xtitle("{stSymbol: d}") title("Local to Zero Approach") /*
			*/ note("Methodology described in Conley et al. (2012)")
		}
